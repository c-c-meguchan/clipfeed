...
private struct AppAccentColorKey: EnvironmentKey {
    static let defaultValue: Color = AppSettings.accentColor(for: "blue")
}
...
struct AccentTintView<Content: View>: View {
    @AppStorage(AppSettings.accentColorKey) private var accentId: String = "blue"
    @ViewBuilder let content: () -> Content

    var body: some View {
        let color = AppSettings.accentColor(for: accentId)
        content()
            .tint(color)
